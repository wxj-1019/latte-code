...
| `/goal` | 设置自主执行目标（自动调研、计划、执行） |
| `/exit` | 退出程序 |

### Goal 命令配置

`/goal` 命令支持自主执行模式，可通过以下环境变量进行配置：

| 变量名 | 默认值 | 说明 |
|--------|--------|------|
| `GOAL_MAX_TURNS` | 50 | 目标执行的最大轮次 |
| `GOAL_MAX_TURNS_LIMIT` | 200 | 最大轮次上限 |
| `GOAL_MAX_ZERO_TOOL_CALLS` | 5 | 空闲抑制阈值（连续无工具调用轮次） |
| `GOAL_REFLECTION_INTERVAL` | 5 | 自我反思间隔（每N轮反思一次） |

**示例：**
```bash
# 设置较大的轮次限制
export GOAL_MAX_TURNS=100

# 更频繁的反思（每3轮）
export GOAL_REFLECTION_INTERVAL=3
```
