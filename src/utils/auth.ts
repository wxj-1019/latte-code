...
import { getAPIProvider, getOpenAISubMode } from 'src/utils/model/providers.js'
import {
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // Latte-compatible auth should bypass Anthropic OAuth entirely.
  // Respect the explicit LATTE_API_KEY env var first, then fall back to any
  // saved custom model configuration from the global config.
  if (process.env.LATTE_API_KEY?.trim()) {
    return false
  }

  if ((getGlobalConfig().customModels?.length ?? 0) > 0) {
    return false
  }
  ...
  const is3P =
    apiProvider === 'openai'
      ? openAISubMode === 'codex' || openAISubMode === 'compatible'
...
/** Check if using a non-first-party API provider */
