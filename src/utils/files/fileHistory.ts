...
export async function fileHistoryTrackEdit(
  ...
  // the deterministic {hash}@v1 backup on every repeat call â€?a second
  ...
  updateFileHistoryState((state: FileHistoryState) => {
    try {
      ...
      // backup's Date/string fields â€?O(n) cost to add one entry.
...
export async function fileHistoryMakeSnapshot(
  ...
  // â€?if fileHistoryTrackEdit added a file during phase 2's async window, it
...
function compareStatsAndContent<T extends boolean | Promise<boolean>>(
  ...
  // own read errors â€?a try/catch here is dead for async callbacks anyway.
...
async function createBackup(
  ...
  // copy. Separates "source missing" from "backup dir missing" cleanly â€?
