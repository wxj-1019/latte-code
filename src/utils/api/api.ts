...
export async function toolToAPISchema(
  ...
  // call â€?name-only keying returned a stale schema (5.4% â†?51% err rate, see
  ...
  // shapes. Proxy gateways (ANTHROPIC_BASE_URL â†?LiteLLM â†?Bedrock) reject
  ...
  // schemas pass through â€?including fields added in the future.
...
export function normalizeToolInput<T extends Tool>(
  ...
  switch (tool.name) {
    ...
    case FileWriteTool.name: {
      ...
      // Markdown uses two trailing spaces as a hard line break â€?don't strip.
