 ...
 * is checked via instanceof because minified builds mangle class names â€?
 ...
 * Use when the error flows to the model as a tool_result â€?full stack
 ...
 * structurally unreachable â€?use in catch blocks after fs operations to
 * distinguish expected "nothing there / no access" from unexpected errors.
 *
 * Covers:
 *  ENOENT    â€?path does not exist
 *  EACCES    â€?permission denied
 *  EPERM     â€?operation not permitted
 *  ENOTDIR   â€?a path component is not a directory (e.g. a file named
 *              `.claude` exists where a directory is expected)
 *  ELOOP     â€?too many symlink levels (circular symlinks)
 */
...
export type AxiosErrorKind =
  | 'auth' // 401/403 â€?caller typically sets skipRetry
 ...
 * Replaces the ~20-line isAxiosError â†?401/403 â†?ECONNABORTED â†?ECONNREFUSED
