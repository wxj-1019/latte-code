...
import { getAPIProvider, getOpenAISubMode } from './model/providers.js';
...
export function buildAPIProviderProperties(): Property[] {
  ...
  const compatibleConfig = getResolvedCustomModelConfig();
  const properties: Property[] = [];
  if (apiProvider !== 'firstParty') {
    const providerLabel = apiProvider === 'openai' ? openAISubMode === 'compatible' ? 'OpenAI-compatible API' : 'OpenAI Codex' : {
      bedrock: 'AWS Bedrock',
  ...
  } else if (apiProvider === 'openai' && openAISubMode === 'compatible') {
    if (compatibleConfig?.baseURL) {
      properties.push({
        label: 'Compatible base URL',
        value: compatibleConfig.baseURL
      });
    }
    if (compatibleConfig?.name) {
      properties.push({
        label: 'Compatible model',
        value: `${compatibleConfig.name} (${compatibleConfig.model})`
      });
    }
    if (compatibleConfig?.apiMode) {
      properties.push({
        label: 'Compatible mode',
        value: compatibleConfig.apiMode
      });
    }
