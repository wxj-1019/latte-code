...
export function registerPluginSubcommands(program: CommanderCommand): void {
  ...
  const marketplaceCmd = pluginCmd.command('marketplace')
    .description('Manage Claude Code marketplaces')
    .configureHelp(createSortedHelpConfig());

  marketplaceCmd.command('add <source>')
    .description('Add a marketplace from a URL, path, or GitHub repo')
    .addOption(coworkOption())
    .option('--sparse <paths...>', 'Limit checkout to specific directories via git sparse-checkout (for monorepos). Example: --sparse .claude-plugin plugins')
    .option('--scope <scope>', 'Where to declare the marketplace: user (default), project, or local')
    .action(async (source: string, options: { cowork?: boolean; sparse?: string[]; scope?: string }) => {
  ...
  marketplaceCmd.command('remove <name>')
    .alias('rm')
  ...
  marketplaceCmd.command('update [name]')
  ...
  pluginCmd.command('install <plugin>')
    .alias('i')
  ...
  pluginCmd.command('uninstall <plugin>')
    .alias('remove')
    .alias('rm')
    ...
    .addOption(coworkOption())
    .action(async (plugin: string, options: { scope?: string; cowork?: boolean; keepData?: boolean }) => {
      const { pluginUninstallHandler } = await import('../../cli/handlers/plugins.js');
      await pluginUninstallHandler(plugin, options);
    });

  // Plugin enable command
  pluginCmd.command('enable <plugin>')
  ...
  pluginCmd.command('disable [plugin]')
  ...
  pluginCmd.command('update <plugin>')
